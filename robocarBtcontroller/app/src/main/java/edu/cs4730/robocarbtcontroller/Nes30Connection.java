...
public class Nes30Connection {
  ...
  public Nes30Connection(Context context, String deviceAddress) {
    ...
    if (bluetoothAdapter == null) {
      logger("This device does not support Bluetooth.");
    } else if (!isEnabled()) {
      logger("Bluetooth isn't enabled, enabling: %b" + bluetoothAdapter.enable());
    }
  }
  ...
  private final BroadcastReceiver receiver = new BroadcastReceiver() {
    public void onReceive(Context context, Intent intent) {
      ...
      if (BluetoothDevice.ACTION_FOUND.equals(action)) {
        ...
        logger("Discovery has found a device: %d/%s/%s" + bondState+" "+ foundName+" "+ foundAddress);
        if (isSelectedDevice(foundAddress)) {
          createBond(device);
        } else {
          logger("Unknown device, skipping bond attempt.");
        }
      } else if (BluetoothDevice.ACTION_BOND_STATE_CHANGED.equals(action)) {
        int state = intent.getIntExtra(BluetoothDevice.EXTRA_BOND_STATE, -1);
        switch (state) {
          case BluetoothDevice.BOND_NONE:
            logger("The remote device is not bonded.");
            break;
          case BluetoothDevice.BOND_BONDING:
            logger("Bonding is in progress with the remote device.");
            break;
          case BluetoothDevice.BOND_BONDED:
            logger("The remote device is bonded.");
            break;
          default:
            logger("Unknown remote device bonding state.");
            break;
        }
      }
    }
...
//      device.setPairingConfirmation(false);
    ...
    logger("Creating bond with: %s/%s/%b"+" "+ device.getName()+" "+ device.getAddress()+" "+ result);
    return result;
  }
  ...
  public void removeBond(BluetoothDevice device) {
    try {
      logger("Removing bond.");
      Method m = device.getClass().getMethod("removeBond", (Class[]) null);
      m.invoke(device, (Object[]) null);
    } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
      logger( "Failed to remove bond.");
    }
  }
  public void logger(String item) {
    Log.d("NESConn", item);
  }
}
